    ...
    <div className="space-y-2">
      ...
      <div className="relative flex items-center">
        ...
        <input
          ...
          placeholder="가게 이름, 읍·면 검색 (예: 문장리)"
          className="w-full pl-10 pr-20 py-2 bg-white border border-slate-200 rounded-xl text-sm placeholder-slate-400 text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm transition-all"
        />
        ...
        <div className="absolute right-2 flex items-center gap-1">
          ...
          {onLocateMe && (
            <button
              onClick={onLocateMe}
              disabled={isLocating}
              className={`flex items-center gap-1 px-2.5 py-1 text-xs font-semibold rounded-lg border whitespace-nowrap flex-shrink-0 transition-all ${
                isLocating
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 no-scrollbar text-xs">
        ...
        {QUICK_TAGS.map((tag) => (
          <button
            ...
            className={`px-2.5 py-1 rounded-full whitespace-nowrap flex-shrink-0 transition-colors border ${
